...
dev-rebuild-clean: ## 开发环境完全清理重建（使用 docker-compose.local.yml）
	...
	@read -p "确定要继续吗? [y/N] " confirm && [ $$confirm = "y" ] || exit 0
	@echo "$(BLUE)🧹 完全清理中...$(NC)"
	@project_images=$$(docker-compose -f docker-compose.local.yml images -q backend frontend data-service agent | sort -u); \
	...
	while [ $$try -le $$max_tries ]; do \
		stock_count=$$(docker-compose -f docker-compose.local.yml exec -T postgresql psql -U $${POSTGRES_USER:-koduck} -d $${POSTGRES_DB:-koduck_dev} -Atc "SELECT COUNT(*) FROM stock_basic" 2>/dev/null | tr -d '[:space:]'); \
		pair_count=$$(docker-compose -f docker-compose.local.yml exec -T postgresql psql -U $${POSTGRES_USER:-koduck} -d $${POSTGRES_DB:-koduck_dev} -Atc "SELECT COUNT(DISTINCT symbol || '|' || timeframe) FROM kline_data WHERE market='AShare'" 2>/dev/null | tr -d '[:space:]'); \
