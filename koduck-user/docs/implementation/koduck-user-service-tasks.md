...
**验收标准:**
- [x] Repository 方法覆盖设计文档核心用例
- [x] 用户搜索支持 keyword/status/page/size/sort
...
**验收标准:**
- [x] Service 方法覆盖设计文档 6.1 节主要流程
- [x] 业务规则（幂等、邮箱重验证）正确实现
- [x] 事务边界合理（读写分离、写事务完整）
...
**验收标准:**
- [x] 角色与权限服务覆盖设计文档 4.1.2/4.1.3
- [x] 删除与变更行为有明确约束
- [x] 权限聚合结果可用于网关透传与服务鉴权
...
**验收标准:**
- [x] 路由与设计文档 4.1.1 完全一致
- [x] 参数校验和错误处理正确
- [x] 当前用户接口不依赖前端传入 userId
...
**验收标准:**
- [x] 路由覆盖设计文档 4.1.2、4.1.3
- [x] 接口权限要求在控制层与业务层均有体现
- [x] 返回值结构与 DTO 定义一致
...
**验收标准:**
- [x] 与 koduck-auth 的内部 API 契约测试通过
- [x] 错误码与响应结构无歧义
- [x] 关键调用路径具备回归测试
...
**验收标准:**
- [x] 未开启开关时，koduck-user 不存在到 auth 的主动调用链路
- [x] 开启后所有调用均通过 APISIX 地址而非 auth 直连地址
- [x] `apikey` 缺失或错误时，链路按预期失败并记录审计日志
...
**验收标准:**
- [x] 未认证请求无法访问公开 API
- [x] 未携带合法内部 key 的请求无法访问 `/internal/*`
- [x] 权限不足请求返回预期状态码（403）
...
### Task 7.4: 数据迁移与灰度上线
**状态:** Deferred / Won't Do（当前阶段不执行）

**说明:**
- 当前 `koduck-user` 处于开发与联调阶段，暂无“旧库 -> user_db”生产迁移窗口。
- 为保持任务可追溯性，本任务保留在文档中，不删除；待进入正式上线准备阶段再评估是否恢复执行。
