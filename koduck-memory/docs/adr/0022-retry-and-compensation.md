...
4. **重试包装器**：
   ...
   - `SummaryTaskRunner.run` 被包装为可重试的异步操作
   - 主链路 `tokio::spawn` 中调用 `with_retry(runner.run(job))`
...
2. 最终失败的记录可被查询，支持事后补偿和告警。
3. 主链路 `SummarizeMemory` RPC 仍然只做任务投递，重试全部在后台完成，不影响响应延迟。
4. Metrics 让运维能监控重试率和失败率。
