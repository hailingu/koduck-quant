...
/**
 * StringArrayField - ArrayField for string arrays.
 * @param props
 */
export const StringArrayField: React.FC<ArrayFieldProps<string>> = (props) => (
  <ArrayField<string> {...props} />
);

StringArrayField.displayName = "StringArrayField";

/**
 * NumberArrayField - ArrayField for number arrays.
 * @param root0
 * @param root0.itemSchema
 */
export const NumberArrayField: React.FC<ArrayFieldProps<number>> = ({ itemSchema, ...props }) => {
  const defaultItemSchema: ExtendedFormFieldSchema = itemSchema ?? {
    type: "number",
  };

  const NumberItemRenderer: React.FC<ArrayItemRenderProps<number>> = ({
    value,
    index,
    onChange,
    onRemove,
  ...
  return (
    <ArrayField<number>
      ...
      itemRenderer={NumberItemRenderer}
    />
  );
};
