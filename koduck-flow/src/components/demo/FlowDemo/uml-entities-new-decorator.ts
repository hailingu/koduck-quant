...
export class UMLLineEntity extends UMLEdgeEntity<LineEdge> {
  ...
  constructor(args?: IEntityArguments) {
    ...
    const initialSourceId = typeof a.sourceId === "string" ? (a.sourceId as string) : undefined;
    const initialTargetId = typeof a.targetId === "string" ? (a.targetId as string) : undefined;
    const initialSourcePort =
      typeof a.sourcePort === "string" ? (a.sourcePort as string) : undefined;
    const initialTargetPort =
      typeof a.targetPort === "string" ? (a.targetPort as string) : undefined;
    const initialSourcePortIndex =
      typeof a.sourcePortIndex === "number" ? (a.sourcePortIndex as number) : undefined;
    const initialTargetPortIndex =
      typeof a.targetPortIndex === "number" ? (a.targetPortIndex as number) : undefined;
    ...
    const edgeState = isEdgeState(a.edgeState)
      ? (a.edgeState as "active" | "inactive" | "disabled")
      : undefined;
  ...
  private resolveEndpointPosition(
    ...
    const entity = runtime.EntityManager.getEntity(endpoint.nodeId) as UMLEntity | undefined;
    if (!entity) {
  ...
  private computeRenderEndpoints(): {
    ...
    if (sourceResolved && targetResolved && this.data) {
      if (!this.data.position) {
        this.data.position = { x: start.x, y: start.y };
      } else {
  ...
  protected override createEdge(args?: IEntityArguments): LineEdge {
    ...
    const targetId = typeof a.targetId === "string" ? (a.targetId as string) : undefined;
    const sourcePort = typeof a.sourcePort === "string" ? (a.sourcePort as string) : undefined;
    const targetPort = typeof a.targetPort === "string" ? (a.targetPort as string) : undefined;
  ...
  setConnection(config: {
    ...
    const nextSourcePort =
      config.sourcePort === undefined ? this.data?.sourcePort : config.sourcePort;
    const nextTargetPort =
      config.targetPort === undefined ? this.data?.targetPort : config.targetPort;

    const nextSourcePortIndex =
      config.sourcePortIndex === undefined ? this.data?.sourcePortIndex : config.sourcePortIndex;
    const nextTargetPortIndex =
      config.targetPortIndex === undefined ? this.data?.targetPortIndex : config.targetPortIndex;
  ...
  getConnection(): {
    ...
    const resolvedSourcePort =
      sourceEndpoint?.port ??
      (this.data && this.data.sourcePort !== undefined ? this.data.sourcePort : undefined);
    ...
    const resolvedTargetPort =
      targetEndpoint?.port ??
      (this.data && this.data.targetPort !== undefined ? this.data.targetPort : undefined);
