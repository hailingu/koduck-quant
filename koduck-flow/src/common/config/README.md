# DuckFlow Configuration System

## 概述

DuckFlow 配置系统提供了一个统一的配置管理机制，支持多种配置源的优先级合并和运行时覆盖。
...
1. `./duckflow.config.json`
2. `./config/duckflow.config.json`
3. `./src/config/duckflow.config.json`
...
- `DUCKFLOW_EVENT_BATCH_SIZE`: 批处理大小
- `DUCKFLOW_EVENT_BATCH_INTERVAL`: 批处理间隔
- `DUCKFLOW_EVENT_MAX_QUEUE_SIZE`: 最大队列大小

### 渲染配置

- `DUCKFLOW_RENDER_FRAME_RATE`: 帧率
- `DUCKFLOW_RENDER_CACHE_TTL`: 缓存TTL

### 性能配置

- `DUCKFLOW_PERFORMANCE_ENABLE_PROFILING`: 启用性能分析 (true/false)
- `DUCKFLOW_PERFORMANCE_METRICS_INTERVAL`: 指标间隔
...
在浏览器环境中，可以通过全局变量 `window.DUCKFLOW_CONFIG` 提供配置：

```javascript
window.DUCKFLOW_CONFIG = {
